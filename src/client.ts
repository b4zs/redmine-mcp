    ...
    if (project_ids && project_ids.length > 0) {
      ...
      if (statusIdArray !== undefined) {
        ...
        if (statusIdArray.length === 1) {
          ...
          if (s === 'open') { qb.addFilter('status_id', 'o'); }
          else if (s === 'closed') { qb.addFilter('status_id', 'c'); }
          else if (s === '*') { qb.addFilter('status_id', '*'); }
          else { qb.addFilter('status_id', '=', [validator.normalizeStatus(statusIdArray[0])]); }
        } else {
