...
export function registerAllTools(server: FastMCP, client: RedmineClient) {
  server.addTool({
    ...
    description:
      "List issues assigned to a user. Default user is 'me' (API key owner). Use list_users to find numeric user_id. For status_id in list context, use: 'open' (default), 'closed', '*' (all), or numeric ID. Response includes total count and pagination info (offset, limit).",
  ...
  server.addTool({
    name: 'update_issue',
    description:
      "Update an issue's status, assignee, and/or progress. Pass notes to leave a comment in the same call. status_id must be numeric (use get_issue to see current status). assigned_to_id must be numeric from list_users or 'me'.",
    parameters: z.strictObject({
  ...
  server.addTool({
    ...
    description:
      'Log time spent on an issue or project. Required: either issue_id or project_id (not both), and hours. Optional: activity_id (numeric; use get_issue_time_entries to see valid values), spent_on (YYYY-MM-DD; defaults to today). activity_id and spent_on must use numeric IDs and valid dates.',
    parameters: z.strictObject({
  ...
  server.addTool({
    name: 'list_projects',
    description:
      'List all projects in the Redmine instance. Optionally search by project name, identifier, or description. Response includes total count and pagination info (offset, limit) to support manual pagination.',
    parameters: z.strictObject({
