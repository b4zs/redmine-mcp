...
export function registerAllTools(server: FastMCP, client: RedmineClient) {
  ...
  server.addTool({
    name: 'list_projects',
    description:
      'List all projects in the Redmine instance. Optionally search by project name, identifier, or description. Response includes total count and pagination info (offset, limit) to support manual pagination. With paginate=true, returns merged list of all available pages. With save_to_file=true, saves result to /tmp and returns file path.',
    parameters: z.strictObject({
